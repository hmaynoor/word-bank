...
export default function Page() {
  ...
  async function saveMeaning(m: Meaning) {
    ...
    setStatus(error ? error.message : "Saved ツ");
    await refreshCards();
  }
...
function nextQuestion() {
  ...
  async function mark(correct: boolean) {
    ...
    setStatus(error ? error.message : correct ? `Nice ツ  — due again in ${days} days` : "Again Soon");
    setRevealed(false);
    setMcqChoiceId(null);
    setMcqResult(null);
    nextQuestion();
...
useEffect(() => {
  ...
  if (!session) {
    return (
      ...
      >
        ...
        <p style={{ opacity: 0.8 }}>Log in once and your words sync between your laptop and phone.</p>
  ...
  return (
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        ...
        <button onClick={signOut} style={{ fontFamily: "Garamond", padding: "10px 12px" }}>Sign Out</button>
      </div>

      <div style={{ display: "flex", gap: 8, marginTop: 16, flexWrap: "wrap" }}>
        <button onClick={() => setTab("add")} style={{ fontFamily: "inheret", padding: "8px 10px" }}>
          Add
        </button>
        <button onClick={() => { setTab("review"); setReviewIndex(0); setRevealed(false); }} style={{ fontFamily: "inheret", padding: "8px 10px" }}>
          Review
        </button>
        <button onClick={() => setTab("bank")} style={{ fontFamily: "inheret", padding: "8px 10px" }}>
          Word Bank
        </button>
            ...
            if (!c) {
              return (
                <div style={{ opacity: 0.8 }}>
                  {cards.length ? "Nothing due ٩(˘◡˘)۶ Switch to Random?" : "No words saved yet."}
                </div>
              );
            }
              ...
              <div style={{ border: "1px solid #F6F4F0", borderRadius: 14, padding: 14 }}>
                ...
                {testMode === "mcq" && (
                  <div style={{ marginTop: 12 }}>
                    ...
                    {mcqResult === "correct" && <div style={{ marginTop: 10 }}>Correct ツ</div>}
                    {mcqResult === "wrong" && (
                ...
                {testMode !== "mcq" && (
                  <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => mark(false)} disabled={!revealed}>✘ Not Sure</button>
                  </div>
                )}
              </div>
            );
